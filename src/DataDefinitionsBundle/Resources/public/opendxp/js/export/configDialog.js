...
opendxp.registerNS('opendxp.plugin.datadefinitions.export.configDialog');

opendxp.plugin.datadefinitions.export.configDialog = Class.create({
    getConfigDialog: function (fromColumn, record, config) {
        ...
        fieldSetItems.push(new Ext.form.ComboBox({
            ...
            store: opendxp.globalmanager.get('data_definitions_getters'),
            valueField: 'getter',
            displayField: 'getter',
            queryMode: 'local',
            listeners: {
    ...
    getGetterPanelLayout: function (type) {
        if (type) {
            ...
            if (opendxp.plugin.datadefinitions.getters[type]) {
                this.getter = new opendxp.plugin.datadefinitions.getters[type]();
