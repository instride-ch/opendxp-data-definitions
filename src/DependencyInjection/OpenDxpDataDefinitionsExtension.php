...
namespace Instride\Bundle\OpenDxpDataDefinitionsBundle\DependencyInjection;
...
class OpenDxpDataDefinitionsExtension extends Extension implements PrependExtensionInterface
{
    /**
     * @throws Exception
     */
    public function load(array $configs, ContainerBuilder $container): void
    {
        ...
        if (array_key_exists('EcommerceStoreBundle', $bundles)) {
            $loader->load('opendxp_ecommerce.yaml');
        }

        $loader->load('services.yaml');

        if (class_exists(HttpFactory::class)) {
            $loader->load('guzzle_psr7.yaml');
        }
    ...
    {
        return 'opendxp_data_definitions';
    }

    public function prepend(ContainerBuilder $container): void
    {
        LocationAwareConfigRepository::loadSymfonyConfigFiles($container, 'opendxp_data_definitions', 'export_definitions');
        LocationAwareConfigRepository::loadSymfonyConfigFiles($container, 'opendxp_data_definitions', 'import_definitions');
        ...
        if (class_exists(JMS\SerializerBundle\JMSSerializerBundle::class)) {
            $container->prependExtensionConfig('jms_serializer', [
                'metadata' => [
                    'directories' => [
                        'data-definitions' => [
                            'namespace_prefix' => 'OpenDxpDataDefinitionsBundle',
                            'path' => '@OpenDxpDataDefinitionsBundle/Resources/config/serializer',
                        ],
                    ],
                ],
            ]);
